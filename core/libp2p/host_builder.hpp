...
namespace libp2p {
  ...
  class HostBuilder {
   public:
     ...
     */
    HostBuilder &addTransport(detail::sptr<transport::TransportAdaptor> tr);
     ...
     * @note if no muxers are added, Yamux will be used; it will THROW AWAY all
     * streams
     */
     ...
     */
    outcome::result<std::shared_ptr<Host>> build();
